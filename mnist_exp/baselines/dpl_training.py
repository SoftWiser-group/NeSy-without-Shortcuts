...
# for 'extra', we set lr = 5e-4 to avoid nan
# for 'gm', we set lr = 1e-3
parameters = {
    "method": ["exact"],
    "run": [0],
}
...
train_set = MNIST("train", "rot")
test_set = MNIST("test")

network = MNIST_Net()

net = Network(network, "mnist_net", batching=True)
net.optimizer = torch.optim.Adam(network.parameters(), lr=5e-4)

if configuration["method"] == "exact":
    model = Model("model.pl", [net])
    model.set_engine(ExactEngine(model), cache=True)
elif configuration["method"] == "gm":
    model = Model("tmp.pl", [net])
    model.set_engine(ApproximateEngine(model, 1, geometric_mean, exploration=True))
elif configuration["method"] == "neural":
    model = Model("tmp.pl", [net])
    model.set_engine(ApproximateEngine(model, 1, NeuralHeuristic))
