...
parser = argparse.ArgumentParser(description='PyTorch MNIST Logic Training')
    ...
    for batch_idx, ulab in enumerate(trainloader_unlab):
        ...
        if args.constraint == True:
            for k in range(n_u):
                ...
                #
                # or_res1 = And([LE(probs_u[k,9], 0.95), LE(probs_u[k,6], 0.95)])
                # and_res2 = GE(probs_u[k,6], 0.05)
...
def test(epoch):
    ...
    total_acc = (acc) 
    tau1 = var_or[:,9].detach().cpu().numpy().mean()
    tau2 = var_or[:,10].detach().cpu().numpy().mean()
    ...
    with open("./log/log_{!s}.txt".format(file_name),"a") as f:
        f.write("\n| Epoch #%d\t\t\tLoss: %.4f Acc@1: %.2f%% Cons_Acc1/2: %.2f%%/%.2f%% tau1/tau2: %.2f/%.2f" %(epoch, loss.item(), acc, cons_acc1, cons_acc2, tau1, tau2))
