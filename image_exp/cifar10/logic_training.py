...
best_acc = 0
best_model = None
start_epoch = 1; num_epochs = 300; batch_size = 100 # to avoid incompatible in the last item of dataloader
...
testset = CIFAR10(root='../../data/cifar10', train=False, download=False, transform=transform_test)
num_classes = 10
...
for i in range(10):
    ...
    train_unlab_idx += per_class[i][split+args.num_labeled:split+args.num_labeled+args.num_unlabeled]
...
for epoch in range(start_epoch, start_epoch+num_epochs):
    # optim_w = optim.SGD(net.parameters(), lr=sgd_lr, momentum=0.9, weight_decay=1e-5)
    optim_w = optim.Adam(net.parameters(), lr=adam_lr, weight_decay=1e-5)
    scheduler = None
    # scheduler = lr_scheduler.MultiStepLR(optim_w, milestones=[int(num_epochs/2), int(num_epochs * 3/4)], gamma=0.1)
    # update tau_lr
