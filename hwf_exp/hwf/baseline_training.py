...
def initial_constriants():
    ...
    for batch_idx, sample in enumerate(train_dataloader):
        ...
        index = np.array(range(batch_idx*batch_size, np.minimum((batch_idx+1)*batch_size, num_cons)))
        n_u = outputs.shape[0]
        # constraint_loss = 0
...
def save(acc, e, net, tau, best=False):
    ...
    if not os.path.isdir('checkpoint'):
        os.mkdir('checkpoint')
    if best:
        e = int(100* math.ceil(( float(epoch) / 100)) )
...
def train(epoch):
    ...
    for batch_idx, sample in enumerate(train_dataloader):
        ...
        outputs = probs.reshape(-1, 14)
        targets = label_seq.reshape(-1)
        # labels = F.one_hot(label_seq[:, -1], 14)

        # updates tau
        if args.constraint == True:
            # optim_tau.zero_grad()
            temp_u = probs.clone().detach()
            constraint_loss, _ = cons_loss(temp_u, index, outputs.shape[0])
        ...
        if args.constraint == True:
            constraint_loss, hwx_loss = cons_loss(probs, index, outputs.shape[0])
...
for epoch in range(start_epoch, start_epoch+adam_epochs):
    lr = adam_lr
